...
| `MIN_SKEL` (minimum skeleton bytes to promote a word) | **1** — amended 2026-09-03 (R-MINE-3). ~~**8** — settled; lowering it buys files with near-trivial words~~ was measured against `netStatementReduction`, not review surface. At 1: review surface **1582 → 1086 top / 20999 → 20214 tree**, byte-identity 1037/1037, **0** chunk names orphaned. `skelBytes()` strips holes *before* the floor, so `return ‹expr›;` measured 7 bytes and missed 8 by one (§4B). Env-overridable | `build-lzw-generators.js:91` |
| `SDD_EXPR_SLOT` (expression-level holes) | **off** — measured 2026-09-03 to *raise* review surface to 3,457 top alone, to add nothing on top of `MIN_SKEL=1`, and to orphan 299 chunk names as a canon change. Env-overridable | `engine/operations.js` |
