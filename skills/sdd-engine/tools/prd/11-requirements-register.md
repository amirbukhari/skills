...
| R-ARCH-17 | The renderer **MUST NOT** discard a whole-run word **solely because it covers the run**. | `enlzw.js:121` still enforces the *original* R-MINE-7 (*"a file is never one word"*), which §5D.4 superseded. **Measured: it costs 308 of 943 files their whole-file word, with byte-identity intact either way.** Check: with names present, a file whose run is covered by one word renders as ONE top-level span. **BLOCKED on R-LANG-19** — the amended rule permits a whole-run word only when NAMED, and `word-names.json` is deleted. | §5D.4A |
...
| R-MEAS-6 | The per-file **one-word rate** **MUST** be published by the render producer, beside review surface. | The target of R-ARCH-15 currently has **no producer**: `en-index.json` has no `perFile` and no `oneWordFiles`, so the rate had to be measured by an out-of-band script — the R-MECH-8 shape. Check: both keys present; a run that cannot state the rate is not a passing run. | §5D.4A |
