...
| R-LANG-19 | A **whole-chunk name MUST take precedence** over member composition; composition remains the fallback for words with no whole-chunk name. | `enfile.js:799` currently asserts a span's sentence is composed from its members and *"never invented whole"*, which contradicts one-name-per-chunk and is what emits *"import 1 name from a module then import 1 name from a module"*. **BLOCKED** on §5D.3D §4a/§4b (`word-names.json` is deleted, so chunk names have nowhere to live) — recorded, not holding. | §5D.3D |
| R-LANG-20 | Names **MUST** be assigned in **ascending depth order**, leaves (d=0) first; a word **MUST NOT** be named before every word in its `m` chain is named. | Every composite is `prefix + one leaf` — **0 violations measured across 115,661 (wide) / 126,167 (narrow) entries** — so the dependency relation is a TOTAL ORDER along each chain and a name assigned out of order cannot be grounded in named parts. Check: the naming producer sorts by depth ASC and enumerates a word's components, not only top-level emitted words; `count` orders rows only *within* a depth tier. **SATISFIED 2026-09-01 by `name-words.js` + `engine/naming-plan.js`** — `orderViolations()` refuses a plan with one ungrounded word, and `name --tier N` (N>0) refuses to run while any leaf in the plan is unnamed (§5D.3F). The old worksheet producer `name-words-lzw.js:89` still sorts depth DESCENDING; it is superseded, not the producer. | §5D.3E, §5D.3F |
| R-LANG-21 | Leaf skeletons (d=0) **MUST** be inside whatever naming scope is chosen. | Every chain bottoms out at a leaf, and the tail of every deep word is leaves (**2,659 of 2,659** appended halves measured). A scope that excludes d=0 — as "name depth 1–8" does — leaves the base of every word unnamed. Check: no naming scope excludes d=0 — `naming-plan.tiersOf` throws unless the caller states the leaves are already named, and the CLI refuses a composite tier while any leaf skeleton is unnamed. | §5D.3E, §5D.3F |
| R-LANG-22 | The naming target is **leaves + used shallow words = 2,619 + 2,789 = 5,408 names** (measured 2026-09-01), and it **MUST** be stated as a cost, never as a saving. | It is **more** names than today's 3,237, and that is the honest price of R-ARCH-15's *"words made of words down to leaves"*. What it buys is completeness: at 5,408 every used word is either named directly (d<=8) or is a named prefix plus named leaves (d>=9) — guaranteed by the 0-violations result, so there is no third case and the 448 deep words need no names of their own. Check: any published naming plan states both figures and does not present depth-bounding as a reduction — `naming-plan.summarize()` cannot report the target without today's figure beside it. **RE-MEASURED post-LIFT 2026-09-01 (§5D.3F §2): 2,767 + 3,095 = 5,862 against 3,576 used words**, from the render's own span set (4,788 spans, matching `en-index.json` exactly). The 5,408/3,237 pair was measured over 943 files against `en-index`'s own 1,037; the direction and the ~1.64x ratio are unchanged. | §5D.3E, §5D.3F |
