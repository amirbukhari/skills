...
const STEPS = Object.freeze([
  ...
  { id: "measure", npm: "measure", cmd: ["measure-english.js"], phase: "pipeline", order: 4,
    ...
    disputedOutput: "This step PRINTS reviewSurface, collapseRatioPct and residualStatements to stderr. As of 2026-08-31 those three are known-wrong and flattering: §R row R-MEAS-2 fails because collapsedStatements 22760 exceeds bodyStatements 17852, residualStatements is published as 0 while the manifest's own worstFiles lists per-file residuals of 135/142/101, and reviewSurface exactly equals calls (5731) as a result. sdd-run does NOT parse or republish these — it relays the child's stderr verbatim — but a UI that surfaces them must label them disputed until R-MEAS-2 holds, because they WILL move when it is fixed.",
  ...
  { id: "register", npm: "register", cmd: ["verify-register.js"], phase: "check",
    ...
    coverageWarning: "MANUAL IS NOT A PASS, and a row absent from the runner is not a row that holds. Measured 2026-08-31 21:30 UTC: 35 rows mechanized of 119 in the register — 31 hold, 1 FAILS, 3 manual. A UI that renders this as \"all green\" is lying about the register: ~84 rows are UNKNOWN, not passing. Show the fraction mechanized next to the result, always. These counts go stale every time a row is mechanized — the runner prints its own summary line and `--json` carries `summary`; treat THAT as authoritative, not this string.",
    exitCodeNote: "Exits 1 while any mechanized row fails, and one does (R-MEAS-2). A non-zero here is the register working, not the runner breaking — read `summary.fails` before rendering it as an error.",
    knownRed: "As of 2026-08-31 R-MEAS-2 FAILS: collapsedStatements 22760 > bodyStatements 17852 on the live manifest. Left unfixed deliberately and escalated to Amir — the fix moves a headline metric. Owned by another lane; do not silently repair it.",
    measuredMs: 720, measuredNote: "measured 2026-08-31 on the self-hosting corpus, 1037 files",
